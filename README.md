...
- 连续语音面试、可选双向实时 WebRTC、回答依据与口语观测评分，以及可商用的原创 liuli 动态角色。
- 简历图片和扫描 PDF 识别、AI 优化建议、Word/PDF 导出。
- 企业岗位发布、按岗位投递、候选人筛选、面试邀约与结果跟进。
- 页面切换和动画性能优化；实时模型需按 [配置说明](docs/interview-realtime.md) 开通。
