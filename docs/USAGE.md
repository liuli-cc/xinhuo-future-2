...
学生：注册并审核通过后，先完善账号资料；在成长地图提交任务佐证或在能力画像添加佐证；审核通过后画像和进度自动更新；在职业工作台粘贴真实岗位、保存快照、计算匹配、建立投递记录；从投递记录点“岗位模拟面试”会自动带入岗位，复盘后可把改进项带回成长地图。面试页可用免费本地计划，也可由管理员配置服务端模型。

全局快捷导航：学生页面按 `⌘ K`（Windows/Linux 为 `Ctrl K`），可搜索并直达成长地图、能力画像、成长决策、实习就业和模拟面试。账号菜单内可选“完整动效 / 减少动效 / 跟随系统”。

成长任务衔接：AI 成长建议点“加入成长地图”，或公开成长资源点“加入成长地图”，会生成当前学期的自定义任务。重复点击使用稳定任务编号，不会堆叠重复任务。
