...
export function buildInterviewReportMarkdown(
  ...
  const details = report.scoredAnswers.map((item, index) => {
    ...
    return [
      ...
      `- 评估来源：${item.evaluationNote || (item.evaluationSource === "model-evidence" ? "AI 证据评估" : "提纲评分")}`,
  ...
  return [
    ...
    `- 平均语速：${report.expressionSummary.answersWithVoice ? report.expressionSummary.averageWordsPerMinute : "—"} 字/分`,
    `- 平均停顿占比：${report.expressionSummary.answersWithVoice ? report.expressionSummary.averagePauseRatio : "—"}%`,
    `- 平均开口前思考：${report.expressionSummary.answersWithVoice ? report.expressionSummary.averageThinkingSeconds : "—"} 秒`,
    `- 口头语总计：${report.expressionSummary.answersWithVoice ? report.expressionSummary.totalFillers : "—"} 次（${report.expressionSummary.answersWithVoice ? report.expressionSummary.fillerWordsPerMinute : "—"} 次/分）`,
    ...
    "",
    "---",
    "评估依据：回答原文、练习量表与可用语音指标；启用 AI 时，带原文证据的模型评估参与计分。语气词以转写可见内容统计，停顿为浏览器采样估计。分数用于模拟练习。",
    "",
  ].join("\n");
}
