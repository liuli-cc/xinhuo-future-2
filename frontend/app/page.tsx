...
import { apiFetch, updateCachedUserProfile } from "@/modules/shared/api/bmob-api";

import { FormEvent, useEffect, useState } from "react";
import { nameIssue, passwordIssue, staffIdIssue, studentIdIssue, validEmail } from "@/modules/shared/auth/auth";
...
// Local-only preview profile. It is enabled through .env.local and is never
// active in a production build, so the real authentication flow stays intact.
...
const previewProfile = {
  ...
  targetRole: "AI 产品经理",
  developmentTrack: "exploration",
  interests: ["人工智能", "产品设计", "创新实践"],
  consentAt: Date.now(),
};
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (previewMode) {
      updateCachedUserProfile(previewProfile);
  ...
  return <main className="setup-page auth-page">
    <button className="setup-back" onClick={goBack}><ArrowLeft size={18} weight="bold" />返回</button>
    <header className="setup-brand"><span className="setup-logo"><FireSimple size={20} weight="fill" /></span><b>薪火</b><em>AI 大学生成长平台</em></header>
    <section className={`setup-shell auth-shell ${mode === "register" ? "registering" : ""}`}>
      <div className="setup-story auth-story">
        <span className="setup-kicker"><Sparkle size={16} weight="fill" /> PERSONAL GROWTH CLOUD</span>
        ...
        <div className="privacy-note"><ShieldCheck size={21} weight="duotone" /><div><b>账号数据加密传输</b><small>密码不会明文保存，学生之间的数据相互隔离</small></div></div>
      </div>
      ...
      <section className="profile-card auth-card">
        ...
        {mode === "login" ? <form onSubmit={submitLogin}>
          ...
          <div className="form-grid auth-form-grid">
            <label className="wide"><span>学号 / 工号</span><div><i><IdentificationCard size={17} /></i><input value={login.studentId} onChange={event => setLogin(current => ({ ...current, studentId: event.target.value }))} placeholder="请输入 6-20 位数字账号" inputMode="numeric" autoComplete="username" /></div></label>
            <label className="wide"><span>密码</span><div><i><LockKey size={17} /></i><input type="password" value={login.password} onChange={event => setLogin(current => ({ ...current, password: event.target.value }))} placeholder="请输入密码" autoComplete="current-password" /></div></label>
          </div>
          ...
          <div className="form-grid auth-form-grid">
            <label className="wide"><span>学号 / 工号</span><div><i><IdentificationCard size={17} /></i><input value={recovery.studentId} onChange={event => setRecovery(current => ({ ...current, studentId: event.target.value }))} placeholder="请输入账号" inputMode="numeric" autoComplete="username" /></div></label>
            <label className="wide"><span>账号姓名</span><div><i><User size={17} /></i><input value={recovery.name} onChange={event => setRecovery(current => ({ ...current, name: event.target.value }))} placeholder="请输入注册时姓名" /></div></label>
          </div>
          ...
          <div className="form-grid auth-form-grid">
            <label><span>姓名</span><div><i><User size={17} /></i><input value={registration.name} onChange={event => updateRegistration("name", event.target.value)} placeholder="2-30 个字" autoComplete="name" /></div></label>
            <label><span>{registration.accountType === "teacher" ? "教师工号" : "学号"}</span><div><i><IdentificationCard size={17} /></i><input value={registration.studentId} onChange={event => updateRegistration("studentId", event.target.value)} placeholder={registration.accountType === "teacher" ? "6-12 位数字" : "6-20 位数字"} inputMode="numeric" autoComplete="username" /></div></label>
            <label className="wide"><span>{registration.accountType === "teacher" ? "工作邮箱（必填）" : "邮箱（选填）"}</span><div><i><At size={17} /></i><input type="email" value={registration.email} onChange={event => updateRegistration("email", event.target.value)} placeholder={registration.accountType === "teacher" ? "例如：name@imnu.edu.cn" : "用于后续账号通知"} autoComplete="email" /></div></label>
            <label><span>院系</span><div><i><Buildings size={17} /></i><select value={registration.college} onChange={event => updateRegistration("college", event.target.value)}><option value="" disabled>请选择学院</option>{imnuCollegeNames.map(college => <option key={college} value={college}>{college}</option>)}</select></div></label>
            <label><span>{registration.accountType === "teacher" ? "岗位 / 职称" : "专业"}</span><div><i>{registration.accountType === "teacher" ? <UsersThree size={17} /> : <Student size={17} />}</i><input value={registration.major} onChange={event => updateRegistration("major", event.target.value)} placeholder={registration.accountType === "teacher" ? "例如：讲师" : "例如：计算机科学与技术"} /></div></label>
            <label><span>{registration.accountType === "teacher" ? "负责班级" : "班级"}</span><div><i><UsersThree size={17} /></i><input value={registration.className} onChange={event => updateRegistration("className", event.target.value)} placeholder="例如：计科25-1班" /></div></label>
            {registration.accountType === "student" && <label><span>年级</span><div><i><Student size={17} /></i><select value={registration.grade} onChange={event => updateRegistration("grade", event.target.value)}>{["2022级", "2023级", "2024级", "2025级", "2026级"].map(grade => <option key={grade}>{grade}</option>)}</select></div></label>}
            <label><span>设置密码</span><div><i><LockKey size={17} /></i><input type="password" value={registration.password} onChange={event => updateRegistration("password", event.target.value)} placeholder="10-128 位，含大小写字母和数字" autoComplete="new-password" /></div></label>
            <label><span>确认密码</span><div><i><LockKey size={17} /></i><input type="password" value={registration.confirmPassword} onChange={event => updateRegistration("confirmPassword", event.target.value)} placeholder="再次输入密码" autoComplete="new-password" /></div></label>
          </div>
